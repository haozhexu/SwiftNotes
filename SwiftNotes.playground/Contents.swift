...
// // multiple variables assigned on a single line:
var weightBeforeMarriage = 60.0, weightAfterMarriage = 90.0
var someNumber = 5, someString = "Five for Friday"
...
// ```swift
var ticketPrice: Double = 999.0
var newStuffThisYear: Int = 0
var welcomeMessage: String = "Come spend lots of money and see repeated stuff, your money's welcomed next year as well!"
// // need to be explicit when operation with different types
let totalValue = Double(newStuffThisYear) / ticketPrice
// ```
...
// ```swift
let anInsect = "1", anotherInsect = "2"; print("\(anInsect) and \(anotherInsect) are in the same line")
...
let calculatedMaxUInt16 = Int(pow(2.0, 16.0) - 1)
print("\(maxUInt16 == calculatedMaxUInt16): max value of UInt16 is (2^16)-1")

let maxUInt8 = UInt8.max
let calculatedMaxUInt8 = Int(pow(2.0, 8.0) - 1)
print("\(maxUInt8 == calculatedMaxUInt8): max value of UInt8 is (2^8)-1")

let maxUInt32 = UInt32.max
let calculatedMaxUInt32 = Int(pow(2.0, 32.0) - 1)
print("\(maxUInt32 == calculatedMaxUInt32): max value of UInt32 is (2^32)-1")
// ```
